...
const ContactLinks = ({ children, href, color }) => {
  return (
    <a className={`flex dark:text-gray-200 items-center mb-1 text-lg md:text-xl relative ${colorVariants.transition[color]} after:absolute after:h-[3px] after:w-0 after:bottom-[-3px] after:left-0 hover:after:w-full after:transition-all after:duration-300 cursor-pointer`} href={href} target="_blank" rel="noopener noreferrer">
      {children}
    </a>
  )
}
  ...
  return (
    <section id='contact' name="contact">
      <div className='flex flex-col items-center p-12 group'>
        <h2 className='mb-6 text-4xl dark:text-gray-200'>CONTACT</h2>
