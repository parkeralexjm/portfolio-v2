...
function Projects() {
  ...
  const LinkButton = ({ children, href, color, outline = false, }) => {
    return (
      <a href={href} className="relative px-5 py-3 overflow-hidden font-medium text-gray-800 bg-transparent border border-gray-300 rounded-lg shadow-inner dark:text-gray-200 group/button" target="_blank" rel="noopener noreferrer">
  ...
  return (
    <section id='projects' name='projects' className='flex justify-center p-12 mb-24 bg-gray-100 dark:bg-gray-800 md:mb-36'>
      <div className='container flex flex-col items-center lg:p-12 group'>
        <h2 className='mb-6 text-4xl dark:text-gray-200'>PROJECTS</h2>
            ...
            projects.map(({ name, image, mobileImage, link, github, description, technologies }, index) => {
              return (
                <div key={index} className="flex flex-col items-center mb-6 border-b-2 border-b-yellow-500 lg:border-b-0 lg:basis-1/2 lg:p-4">
                  <h3 className='mb-3 text-2xl dark:text-gray-200'>{name}</h3>
                  ...
                  <div className='flex flex-col items-center flex-grow mb-4'>
                    ...
                    <p className='flex-grow text-justify dark:text-gray-200'>{description}</p>
                  </div>
                  <div className='flex flex-wrap justify-center'>
                    {
                      technologies.map(({ image }, index) => {
                        return (
                          <i key={index} className={`${image} text-2xl mx-1 mb-2 md:text-4xl lg:mb-4 dark:text-gray-200`}></i>
                        )
                      })
                    }
                  </div>
